...
[![Deploy on Zeabur](https://zeabur.com/button.svg)](https://zeabur.com/new?templateUrl=https://raw.githubusercontent.com/basketikun/chatgpt2api/main/zeabur.yaml)

点击上方按钮即可一键部署到 Zeabur，部署时需配置：
- **Domain**：服务访问域名
- **Auth Key**：API 认证密钥

> 如果你想部署自己修改后的代码，Fork 本仓库后推送至你的 GitHub，然后在 Zeabur 控制台创建项目 → 从 GitHub 导入你的仓库即可，Zeabur 会自动识别 Dockerfile 并构建部署。记得在环境变量中配置 `CHATGPT2API_AUTH_KEY` 和 `STORAGE_BACKEND`，并添加 `/app/data` 持久化卷。
